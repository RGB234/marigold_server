...
public class ChatIntegrationTest extends BaseIntegrationTest {
  ...
  void setup() {
    ...
    user1 = userRepository.save(User.builder().nickname("user1").role(Role.ROLE_PERSON).build());
    user2 = userRepository.save(User.builder().nickname("user2").role(Role.ROLE_PERSON).build());
    ...
    chatRoom = chatRoomRepository.save(ChatRoom.create(user1, user2, post));
    participantRepository.save(RoomParticipant.builder().chatRoom(chatRoom).user(user1).build());
    participantRepository.save(RoomParticipant.builder().chatRoom(chatRoom).user(user2).build());
  ...
  void sendAndReceiveMessage() throws InterruptedException, ExecutionException, TimeoutException {
    ...
    // 따라서 순수 웹소켓 엔드포인트를 사용하려면 접속하는 경로인 /webksocket을 URL 뒤에 추가해야 한다.
    ...
    String roomIdStr = TSID.from(chatRoom.getId()).toString();
    ...
    session.subscribe(
        ...
        new StompFrameHandler() {
          ...
          public void handleFrame(StompHeaders headers, Object payload) {
            resultKeeper.complete((ChatMessageDto) payload);
          }
        });
    ...
    assertThat(receivedMessage.getSenderId()).isEqualTo(user1.getId());
  }
}
