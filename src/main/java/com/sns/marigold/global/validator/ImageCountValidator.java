...
public class ImageCountValidator implements ConstraintValidator<ValidImageCount, ImageCountValidatable> {
    private int min;
    private int max;
    ...
    @Override
    public boolean isValid(ImageCountValidatable imageCountValidatable, ConstraintValidatorContext context) {
        int storedImageCount = imageCountValidatable.getImagesToKeep().size();
        
        int newImageCount = imageCountValidatable.getImages() == null || imageCountValidatable.getImages().isEmpty() ? 0
                : (int) imageCountValidatable.getImages().stream().filter(f -> f != null && !f.isEmpty()).count();
